...
Sub AssignSTEPmodel(STEPFileName, RotX, RotY, RotZ, X, Y, Z)
    ...
    DebugLog "[340] PCBLib.CurrentComponent is Nothing=" & (PCBLib.CurrentComponent Is Nothing)
    If Not (PCBLib.CurrentComponent Is Nothing) Then DebugLog "[341] PCBLib.CurrentComponent=" & PCBLib.CurrentComponent.Name
    If PCBLib Is Nothing Then
        DebugLog "[ERROR] PCBLib is Nothing in AssignSTEPmodel"
        Exit Sub
    End If
    Set footprint = PCBLib.CurrentComponent
    DebugLog "[342] footprint is Nothing=" & (footprint Is Nothing)
    ...
    DebugLog "[343] STEP Model is Nothing=" & (Model Is Nothing)
    ...
    DebugLog "[350] STEP SetState done Rot=" & RotX & "/" & RotY & "/" & RotZ & " Z=" & Z

    ' Add model to footprint
    DebugLog "[344] Attach to footprint=" & footprint.Name & " current=" & PCBLib.CurrentComponent.Name
    Set PCBLib.CurrentComponent = footprint
    ...
    DebugLog "[345] STEPmodel is Nothing=" & (STEPmodel Is Nothing)

    ' Move model
    STEPmodel.MoveByXY mmstocoord(x), mmstocoord(y)
    DebugLog "[351] STEP MoveByXY X=" & X & " Y=" & Y
...
Sub CreateComponentInLib(Name,Description,RefDes)
    ...
    DebugLog "[346] SchComponent created=" & (SchComponent Is Nothing)
    ...
    DebugLog "[347] SchComponent RefDes=" & SchComponent.Designator.Text
...
Sub EnsureSchComponentAdded()
    ...
    DebugLog "[348] CurrentSchComponent OwnerPartId=" & SchComponent.CurrentPartID & " DisplayMode=" & SchComponent.DisplayMode
...
If Not (SCHLib Is Nothing) Then DebugLog "[349] Pin attach target=" & SCHLib.CurrentSchComponent.LibReference
